...
document.getElementById("historyUserId")?.addEventListener("change",()=>{
  const confirmInput=document.getElementById("deleteHistoryConfirm");
  const deleteStatus=document.getElementById("deleteHistoryStatus");
...
async function deleteAllHistoryForSelectedUser(){
  const user=selectedUserRecord();
  const input=document.getElementById("deleteHistoryConfirm");
  const status=document.getElementById("deleteHistoryStatus");
  if(!user){
    status.textContent="Pilih user terlebih dahulu.";
    return;
  }

  const typed=normalizeId(input?.value||"");
  if(typed!==normalizeId(user.id)){
    status.textContent=`Ketik User ID "${user.id}" dengan benar untuk konfirmasi.`;
    return;
  }

  let docs=[];
  try{
    docs=await getHistoryDocsForUser(user.id);
  }catch(err){
    console.error(err);
    status.textContent="Gagal membaca riwayat user.";
    return;
  }

  if(!docs.length){
    status.textContent="User ini belum memiliki riwayat KPI.";
    if(input) input.value="";
    return;
  }

  const displayName=user.name && user.name!==user.id ? `${user.name} (${user.id})` : user.id;
  if(!confirm(`Hapus SEMUA ${docs.length} riwayat KPI milik ${displayName}?\n\nTindakan ini tidak dapat dibatalkan.`)) return;

  const button=document.getElementById("deleteAllHistoryBtn");
  if(button) button.disabled=true;
  status.textContent=`Menghapus ${docs.length} riwayat...`;

  try{
    // Hapus bertahap agar tidak mengirim terlalu banyak operasi sekaligus.
    const chunkSize=20;
    for(let i=0;i<docs.length;i+=chunkSize){
      const chunk=docs.slice(i,i+chunkSize);
      await Promise.all(chunk.map(d=>deleteDoc(doc(db,"kpi_history",d.id))));
    }
    status.textContent=`${docs.length} riwayat KPI ${displayName} berhasil dihapus.`;
    if(input) input.value="";
    if(editingHistoryId) cancelEditHistory();
    await loadAdminHistory();
    if(activeIdentity && normalizeId(user.id)===normalizeId(activeIdentity.id)) await loadUserTotals(activeIdentity.id);
  }catch(err){
    console.error(err);
    status.textContent=`Gagal menghapus semua riwayat: ${err.code||err.message||"unknown error"}`;
  }finally{
    if(button) button.disabled=false;
  }
}

document.getElementById("deleteAllHistoryBtn")?.addEventListener("click",deleteAllHistoryForSelectedUser);

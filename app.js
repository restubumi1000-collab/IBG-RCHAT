...
async function handleExcelFile(file){
  ...
  try{
    ...
    excelPreviewRows=rows.filter(r=>Object.values(r).some(v=>String(v).trim()!=="")).map(validateExcelRow);
    if(!excelPreviewRows.length) throw new Error("KPI_INPUT tidak memiliki data.");
    renderExcelPreview();
    const invalid=excelPreviewRows.filter(r=>r.errors.length).length;
    status.textContent=invalid?`Preview selesai. ${invalid} baris perlu diperbaiki sebelum import.`:"Preview selesai. Semua data valid. Belum ada data yang dikirim ke Firestore.";
  }catch(err){
    excelPreviewRows=[];

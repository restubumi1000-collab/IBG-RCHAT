...
async function ensureUserProfile(user) {
  ...
  if (!existing.exists()) {
    // Role selalu user saat profil dibuat otomatis. Admin tetap dikenali dari rules/email admin.
    payload.role = "user";
  }
...
function fillUserSelect(selectId, selectedId = "") {
  ...
  if (selectedId && adminUsers.some(u => u.id === normalizeId(selectedId))) {
    select.value = normalizeId(selectedId);
  }
}
...
async function loadAdminUsers(selectedId = "") {
  ...
  fillUserSelect("adminUserId", selectedId);
  fillUserSelect("kaizenUserId", selectedId);
}

function currentPeriod(){ return localDateParts().period; }
...
document.querySelectorAll(".nav-btn").forEach(b=>b.addEventListener("click",()=>{
  ...
  if(b.dataset.view==="adminPanel" && activeIdentity) loadAdminUsers(activeIdentity.id).catch(console.error);
}));
...
async function loadUserTotals(userId){
  ...
  // Ambil seluruh KPI user, lalu jumlahkan hanya periode tahun berjalan sampai bulan ini.
  // Filter dilakukan di aplikasi agar tidak membutuhkan composite index Firestore.
...
async function loadAllKaizenToToday(userId){
  ...
  docs.sort((a,b)=>String(b.date||"").localeCompare(String(a.date||""))).forEach(d=>{
    ...
    const p=document.createElement("div"); p.className="muted"; p.textContent=d.date||"";
...
function setAdminKpiForm(data={}){
  const map={adminAlpha:"alpha",adminCuti:"cuti",adminTerlambat:"terlambat",adminLdp:"ldp",adminKecelakaan:"kecelakaanKerja",adminKomplain:"komplain",adminProdukNg:"produkNG",adminLembur:"lembur"};
  Object.entries(map).forEach(([el,key])=>{ const input=document.getElementById(el); if(input) input.value=data[key]??0; });
}

async function loadAdminMonthlyKPI(){
  const uid=normalizeId(document.getElementById("adminUserId")?.value||"");
  const period=document.getElementById("adminPeriod")?.value||"";
  if(!uid || !period){ setAdminKpiForm({}); return; }
  try{
    const snap=await getDoc(doc(db,"kpi",`${uid}_${period}`));
    setAdminKpiForm(snap.exists()?snap.data():{});
  }catch(err){
    console.error("Gagal memuat KPI bulanan admin",err);
  }
}

document.getElementById("adminUserId")?.addEventListener("change",loadAdminMonthlyKPI);
document.getElementById("adminPeriod")?.addEventListener("change",loadAdminMonthlyKPI);
document.getElementById("kpiAdminForm")?.addEventListener("submit",async e=>{ e.preventDefault(); const uid=normalizeId(document.getElementById("adminUserId").value), period=document.getElementById("adminPeriod").value, status=document.getElementById("adminKpiStatus"); try{ await setDoc(doc(db,"kpi",`${uid}_${period}`),{userId:uid,period,alpha:num("adminAlpha"),cuti:num("adminCuti"),terlambat:num("adminTerlambat"),ldp:num("adminLdp"),kecelakaanKerja:num("adminKecelakaan"),komplain:num("adminKomplain"),produkNG:num("adminProdukNg"),lembur:num("adminLembur"),updatedAt:serverTimestamp()},{merge:true}); status.textContent="KPI berhasil disimpan. Kolom angka sudah direset ke 0."; setAdminKpiForm({}); }catch(err){ console.error(err); status.textContent="Gagal menyimpan. Periksa Firestore Rules."; } });
document.getElementById("kaizenAdminForm")?.addEventListener("submit", async e => {
  ...
  try {
    ...
    // Langsung perbarui total user jika Kaizen ditambahkan untuk user yang sedang login.
    if (activeIdentity && kaizenUserId === normalizeId(activeIdentity.id)) {
      await loadUserTotals(activeIdentity.id);
    }
...
async function setupKpiUser(user){
  ...
  document.getElementById("adminPeriod").value=currentPeriod();
  document.getElementById("kaizenDate").value=new Date().toISOString().slice(0,10);

  try {
    await ensureUserProfile(user);
  } catch (e) {
    console.warn("User profile could not be registered", e);
  }
  ...
  try{
    ...
    console.warn("Profile role not available",e);
  }

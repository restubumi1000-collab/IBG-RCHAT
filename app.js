...
// ---------- KAIZEN ADMIN ----------
async function getKaizenDocsForUser(userId){
  const qk=query(collection(db,"kaizen"),where("userId","==",normalizeId(userId)));
  const snap=await getDocs(qk);
  return snap.docs.map(x=>({id:x.id,...x.data()})).sort((a,b)=>(b.date||"").localeCompare(a.date||""));
}

function buildAdminKaizenItem(d){
  const item=document.createElement("div"); item.className="kaizen-item";
  const h=document.createElement("h4"); h.textContent=d.title||"Kaizen";
  const p=document.createElement("p"); p.className="muted"; p.textContent=`${d.date||"-"} • ${d.category||"Tanpa kategori"} • ${d.status||"-"}`;
  item.append(h,p);
  if(d.description){ const desc=document.createElement("p"); desc.textContent=d.description; item.append(desc); }
  const actions=document.createElement("div"); actions.className="history-actions";
  const edit=document.createElement("button"); edit.className="back-btn small-btn"; edit.type="button"; edit.textContent="✏️ Edit"; edit.onclick=e=>{e.stopPropagation();startEditKaizen(d);};
  const del=document.createElement("button"); del.className="danger-btn small-btn"; del.type="button"; del.textContent="🗑️ Hapus"; del.onclick=e=>{e.stopPropagation();removeKaizen(d);};
  actions.append(edit,del); item.append(actions); return item;
}

async function loadAdminKaizen(){
  const list=document.getElementById("adminKaizenList"); if(!list) return;
  const userId=normalizeId(document.getElementById("kaizenUserId")?.value||"");
  if(!userId){ list.innerHTML='<div class="muted">Pilih user untuk melihat riwayat Kaizen.</div>'; return; }
  list.innerHTML='<div class="muted">Memuat riwayat Kaizen...</div>';
  try{
    const docs=await getKaizenDocsForUser(userId);
    list.innerHTML="";
    if(!docs.length){ list.innerHTML='<div class="muted">Belum ada Kaizen untuk user ini.</div>'; return; }
    docs.forEach(d=>list.appendChild(buildAdminKaizenItem(d)));
  }catch(err){ console.error(err); list.innerHTML='<div class="error-text">Gagal memuat riwayat Kaizen.</div>'; }
}

document.getElementById("kaizenUserId")?.addEventListener("change",()=>{ cancelEditKaizen(false); loadAdminKaizen(); });
document.getElementById("cancelEditKaizenBtn")?.addEventListener("click",()=>cancelEditKaizen());
...
document.getElementById("kaizenAdminForm")?.addEventListener("submit", async e => {
  ...
  const payload={
    userId: kaizenUserId, date: kaizenDate,
  ...
  try {
    if(editingKaizenId){
      await updateDoc(doc(db,"kaizen",editingKaizenId),payload);
      status.textContent="Kaizen berhasil diperbarui.";
    }else{
      await addDoc(collection(db,"kaizen"), {...payload,createdAt:serverTimestamp()});
    ...
    cancelEditKaizen(false);
    document.getElementById("kaizenUserId").value=kaizenUserId;
    document.getElementById("kaizenDate").value=localDateParts().date;
    await loadAdminKaizen();
  } catch(err) {
    console.error(err); status.textContent = `Gagal menyimpan Kaizen: ${err.code || err.message || "unknown error"}`;
  }
});

function startEditKaizen(d){
  editingKaizenId=d.id;
  document.getElementById("kaizenFormTitle").textContent="Edit Kaizen";
  document.getElementById("saveKaizenBtn").textContent="Simpan Perubahan";
  document.getElementById("cancelEditKaizenBtn").classList.remove("hidden");
  document.getElementById("kaizenUserId").value=normalizeId(d.userId||"");
  document.getElementById("kaizenDate").value=d.date||localDateParts().date;
  document.getElementById("kaizenTitle").value=d.title||"";
  document.getElementById("kaizenCategory").value=d.category||"";
  document.getElementById("kaizenStatus").value=d.status||"Approved";
  document.getElementById("kaizenDescription").value=d.description||"";
  document.getElementById("kaizenAdminForm").scrollIntoView({behavior:"smooth",block:"start"});
}

function cancelEditKaizen(clearStatus=true){
  const keep=document.getElementById("kaizenUserId")?.value||activeIdentity?.id||"";
  editingKaizenId=null;
  document.getElementById("kaizenFormTitle").textContent="Tambah Kaizen";
  document.getElementById("saveKaizenBtn").textContent="Tambah Kaizen";
  document.getElementById("cancelEditKaizenBtn").classList.add("hidden");
  document.getElementById("kaizenAdminForm")?.reset();
  document.getElementById("kaizenUserId").value=normalizeId(keep);
  document.getElementById("kaizenDate").value=localDateParts().date;
  if(clearStatus) document.getElementById("adminKaizenStatus").textContent="";
}

async function removeKaizen(d){
  if(!confirm(`Hapus Kaizen "${d.title||"Kaizen"}" tanggal ${d.date||"-"}?`)) return;
  const status=document.getElementById("adminKaizenStatus");
  try{
    await deleteDoc(doc(db,"kaizen",d.id));
    status.textContent="Kaizen berhasil dihapus.";
    if(editingKaizenId===d.id) cancelEditKaizen(false);
    if(activeIdentity && normalizeId(d.userId||"")===normalizeId(activeIdentity.id)) await loadUserTotals(activeIdentity.id);
    await loadAdminKaizen();
  }catch(err){ console.error(err); status.textContent=`Gagal menghapus Kaizen: ${err.code||err.message||"unknown error"}`; }
}

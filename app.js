...
async function loadKaizen(userId,period){
  const start=`${period}-01`; const [y,m]=period.split("-").map(Number); const next=m===12?`${y+1}-01-01`:`${y}-${String(m+1).padStart(2,"0")}-01`;
  const qk=query(collection(db,"kaizen"),where("userId","==",normalizeId(userId)),where("date",">=",start),where("date","<",next));
  const snap=await getDocs(qk); document.getElementById("kpiKaizen").textContent=snap.size;
  const list=document.getElementById("kaizenList"); list.innerHTML="";
  if(snap.empty){ list.innerHTML='<div class="kaizen-item muted">Belum ada Kaizen pada periode ini.</div>'; return; }
  snap.forEach(ds=>{ const d=ds.data(); const el=document.createElement("div"); el.className="kaizen-item"; const h=document.createElement("h4"); h.textContent=d.title||"Tanpa judul"; const p=document.createElement("div"); p.className="muted"; p.textContent=d.date||""; el.append(h,p); el.onclick=()=>showKaizenDetail(d); list.append(el); });
}
...
document.getElementById("kaizenAdminForm")?.addEventListener("submit",async e=>{ e.preventDefault(); const status=document.getElementById("adminKaizenStatus"); try{ await addDoc(collection(db,"kaizen"),{userId:normalizeId(document.getElementById("kaizenUserId").value),date:document.getElementById("kaizenDate").value,title:document.getElementById("kaizenTitle").value.trim(),category:document.getElementById("kaizenCategory").value.trim(),status:document.getElementById("kaizenStatus").value,description:document.getElementById("kaizenDescription").value.trim(),createdAt:serverTimestamp()}); status.textContent="Kaizen berhasil ditambahkan."; e.target.reset(); }catch(err){ console.error(err); status.textContent="Gagal menambah Kaizen. Periksa Firestore Rules."; } });
